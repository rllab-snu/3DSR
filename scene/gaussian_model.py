...
class GaussianModel(nn.Module):
    ...
    def __init__(self, sh_degree : int):
        ...
        self._xyz_ = torch.empty(0)
        self._features_dc_ = torch.empty(0)
        self._features_rest_ = torch.empty(0)
        self._scaling_ = torch.empty(0)
        self._rotation_ = torch.empty(0)
        self._opacity_ = torch.empty(0)
    ...
    def gaussian2pointcloud(self, radius=-10.0):
        self._scaling = nn.Parameter(torch.full((self._xyz.shape[0], 3), radius, dtype=torch.float, device="cuda").requires_grad_(False))

    def gaussian2point(self, idx, radius=-10.0):
        self._xyz_ = nn.Parameter(torch.unsqueeze(self._xyz[idx], 0).requires_grad_(False))
        self._scaling_ = nn.Parameter(torch.unsqueeze(self._scaling[idx], 0).requires_grad_(False))
        self._features_dc_ = nn.Parameter(torch.unsqueeze(self._features_dc[idx], 0).requires_grad_(False))
        self._opacity_ = nn.Parameter(torch.unsqueeze(self._opacity[idx], 0).requires_grad_(False))
        self._rotation_ = nn.Parameter(torch.unsqueeze(self._rotation[idx], 0).requires_grad_(False))
        self._features_rest_ = nn.Parameter(torch.unsqueeze(self._features_rest[idx], 0).requires_grad_(False))

    @property
    def get_scaling_(self):
        return self.scaling_activation(self._scaling_)
    
    @property
    def get_rotation_(self):
        return self.rotation_activation(self._rotation_)
    ...
    def get_features_(self):
        features_dc = self._features_dc_
        features_rest = self._features_rest_
        return torch.cat((features_dc, features_rest), dim=1)
    
    @property
    def get_opacity_(self):
        return self.opacity_activation(self._opacity_)