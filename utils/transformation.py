...
def pixel_to_world(pixel_pose, config):
    fX_WC = float(config['x_min']) + float(pixel_pose[0]) * float(config['Resolution'])
    fY_WC = float(config['y_min']) + float(pixel_pose[1]) * float(config['Resolution'])
    return [fX_WC, fY_WC]

def world_to_pixel(world_pose, config):
    fX_Pix = (world_pose[0] - float(config['x_min'])) / float(config['Resolution'])
    fY_Pix = (world_pose[1] - float(config['y_min'])) / float(config['Resolution'])
    return [fX_Pix, fY_Pix]

def vector_to_yaw(pose):
    fYaw_rad = np.arctan2(pose[1], pose[0])
    fYaw_deg = fYaw_rad * 180 / np.pi
    return fYaw_deg