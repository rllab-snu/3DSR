...
def render_2(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None, 
    ...
    # Create zero tensor. We will use it to make pytorch return gradients of the 2D (screen-space) means
    screenspace_points = torch.zeros_like(pc.get_xyz_, dtype=pc.get_xyz_.dtype, requires_grad=True, device="cuda") + 0
    try:
        screenspace_points.retain_grad()
    except:
        pass
    ...
    if pipe.compute_cov3D_python:
        ...
        scales = pc.get_scaling_
        rotations = pc.get_rotation_
    ...
    if override_color is None:
        if pipe.convert_SHs_python:
            shs_view = pc.get_features_.transpose(1, 2).view(-1, 3, (pc.max_sh_degree+1)**2)
            dir_pp = (pc.get_xyz_ - viewpoint_camera.camera_center.repeat(pc.get_features_.shape[0], 1))
            dir_pp_normalized = dir_pp/dir_pp.norm(dim=1, keepdim=True)
            sh2rgb = eval_sh(pc.active_sh_degree, shs_view, dir_pp_normalized)
            colors_precomp = torch.clamp_min(sh2rgb + 0.5, 0.0)
        else:
            shs = pc.get_features_
    else:
        colors_precomp = override_color
